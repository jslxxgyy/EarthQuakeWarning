...
**本软件仅供学习交流使用，与 成都高新减灾研究所 中国地震局 无任何直接联系，请勿用于严肃环境，**
...
此软件由 Kengwang 的地震[预警软件](https://github.com/Kengwang/EarthQuakeWarning)分支而来，在前作的基础上进行了全面更新，主要改变：
* 无边框全屏的预警窗口
* 触摸屏操作适配
* 移除复杂的 GNSS 定位，直接使用 Windows 定位 API
* 不再使用 Windows 自带的语音合成，直接使用官方地震预警警报音
* 自定义警报持续时间
* 即时生效的任务栏托盘图标开关
* 在可执行文件中嵌入了图标
* 修改了四川地震局（Wolfx）信息源的逻辑，现在在计算倒计时的时候，会考虑到震源深度
* 在地震发生后的预警期内打开软件依然可以接收到预警
* 托盘图标新增右键退出入口
* 更改中国地震台网（Wolfx）的信息源，从地震情报替换为了地震速报
* 在首次启动软件时，会要求设置经纬度，并移除了默认位于成都的经纬度  

同时，软件移除了告警时的震级阈值和现在已经不可用的四川地震局信息源，升级依赖至.NET 10.0
